  ...
  if(viable){
    if (paso < movimientos.length) {
      ...
      if(movimiento.includes("+")){
        ...
        if(movimiento.includes("O-O-O")){
          console.log("ENROQUE LARGO CON JAQUE"); //enroquel
          enrq = 3;
          var casilla = movimiento.substring(movimiento.length - 2);
          actualizarTablero(tabla, casilla, pieza, turno ? "B" : "N", enrq, movimiento);
        }
        else if(movimiento.includes("O-O")){
          console.log("ENROQUE CORTO CON JAQUE"); //enroquec
          enrq = 2;
          var casilla = movimiento.substring(movimiento.length - 2);
          actualizarTablero(tabla, casilla, pieza, turno ? "B" : "N", enrq, movimiento);
        }
...
function actualizarTablero(tabla, casilla, pieza, turno, enrq, movimiento) {
  ...
  for(var i = 0; i < 8; i++){
    for(var j = 1; j < 9; j++){
      ...
      if ((i + j) % 2 === 0) {
          // Fila par
          tabla.rows[j].cells[i].style.backgroundColor = "#f0f8ff"; // Azul claro
      } else {
          // Fila impar
          tabla.rows[j].cells[i].style.backgroundColor = "#add8e6"; // OAZUL OSCURo
      }
    }
  }
